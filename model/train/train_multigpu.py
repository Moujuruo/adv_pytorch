...
def train_step(images, targets, ddp_network, epoch):
    ...
    ssim_loss = network.config.ssim_loss_weight * (1 - network.ssim(images, g_output))
    
    l2_loss = network.config.l2_loss_weight * F.mse_loss(g_output, images)
    
    pixel_loss = network.config.pixel_loss_weight * F.l1_loss(g_output, images)
    
    g_loss = adv_loss + identity_loss + perturbation_loss + pixel_loss + ssim_loss + l2_loss
