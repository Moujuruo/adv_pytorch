import sys
import time
from model.architecture.advfaces import Generator, NormalDiscriminator, ImprovedGenerator, MultiScaleUNetGenerator
import model.architecture.models as m
from model.architecture.inception_resnet_v1 import InceptionResnetV1
...
from model.utils.pytorch_ssim import SSIM
...
class AdvFaces(nn.Module):
    def __init__(self, config):
        ...
        self.fr_model = m.IR_152((112, 112))
        self.fr_model.load_state_dict(torch.load('model/assets/ir152.pth'))
        self.fr_model.eval()

        # self.ar_model = m.IR_152_attr_all()
        # self.ar_model.load_state_dict(torch.load('assets/ir152_ar.pth'))
        # self.ar_model.eval()

        self.incep_model = InceptionResnetV1(pretrained='vggface2').eval()


        for param in self.fr_model.parameters():
            param.requires_grad = False

        # for param in self.ar_model.parameters():
        #     param.requires_grad = False

        for param in self.incep_model.parameters():
            param.requires_grad = False
